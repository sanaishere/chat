...
export class GroupService {
  ...
  async createGroup(admin: User, body: CreateGroupDto) {
    console.log(body);
    try {
      ...
      this.websocketGateway.emitToClients(
        body.groupMembersId,
        'groupCreated',
        saved,
      );
  ...
  async getMyGroups(user: User) {
    try {
      const foundGroupsAsMembers = await this.groupRepository
        .createQueryBuilder('group')
        .innerJoin('group.groupMembers', 'user')
        .where('user.id = :userId', { userId: user.id })
        .getMany();

      const foundGroupsAsAdmin = await this.groupRepository
        .createQueryBuilder('group')
  ...
  async joinGroup(user: User, groupId: number) {
    ...
    if (group.admin === user || memebers.includes(user.id)) {
      throw new HttpException(
        `you have already join group`,
        HttpStatus.BAD_REQUEST,
      );
    }
    if (group.isPrivate == true) {
      throw new HttpException(
        `you cant join private group`,
        HttpStatus.BAD_REQUEST,
      );
    }
    group.groupMembers.push(user);

    await this.websocketGateway.join({ groupId, userId: user.id });
  ...
  async addToGroup(user: User, body: ChooseUserDto, groupId: number) {
    ...
    if (group.admin.id !== user.id) {
      throw new HttpException(
        'you can not change group members',
    ...
    for (let id of body.userIds) {
      ...
      console.log(user1);
      users.push(user1);
      await this.websocketGateway.join({ groupId, userId: id });
    }
  ...
  async leaveGroup(user: User, groupId: number) {
    ...
    await this.websocketGateway.remove({ groupId, userId: user.id });
  ...
  async checkAdmission(user: User, groupId: number) {
    ...
    if (!(admin.id === user.id)) {
      throw new HttpException(
        'you dont have access to see/change this group',
        HttpStatus.UNAUTHORIZED,
      );
    }
  }

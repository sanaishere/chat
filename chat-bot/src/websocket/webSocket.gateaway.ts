...
export class WebsocketGateway implements OnGatewayConnection {
 ...
 async  handleConnection(client: Socket) {
   ...
   async handleDisconnect(@ConnectedSocket() socket: Socket) {
    const payload=await this.userService.verifyToken(socket.handshake.headers.authorization)
  ...
  async sendMessage(@MessageBody() data: Message,@ConnectedSocket() socket:User) {
  ...
  async sendMessageToGroup(@MessageBody() data: any,@ConnectedSocket() sockets:User[],sender:User) {
    let roomName=`group${data.groupId}`
    ...
    if (!sockets.includes(sender)){
      const senderSocketId=await this.cachService.getValue(sender.id.toString())
...
async happyBirthDay(socket:User){
 const socketId=await this.cachService.getValue(socket.id.toString())
this.server.to(socketId).emit('congrats',`happyBirth day dear ${socket.firstname} `)
}

...
export class MessageService {
  ...
  async sendmessageToUser(
    ...
    if (foundName !== '') {
      ...
      name =
        reciever.firstname + ' ' + reciever.lastname + sender.id + reciever.id;
    }
  ...
  async forwardMessages(
    ...
    for (let messageId of turnToIds) {
      for (let id of body.reciversId) {
        ...
        const chat = await this.sendmessageToUser(
          id,
          sender,
          message.writer,
          body,
  ...
  async sendMessageToGroup(
    ...
    const group = await this.groupRepository.findOne({
      where: { id: groupId },
      relations: { groupMembers: true, admin: true },
    });
    if (!group) {
    ...
    users.push(admin);
    this.webSocketGateAway.sendMessageToGroup(
      { message, groupId },
      users,
      sender,
    );
  ...
  async forwardmessagesToGroup(
    ...
    for (let messageId of messageIds) {
      for (let id of body.reciversId) {
        ...
        const chat = await this.sendMessageToGroup(
          id,
          sender,
          message.writer,
          body2,

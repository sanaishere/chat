...
export class UsersService {
    ...
    async create(body:CreateUserDto,profileSrc:string) {
        ...
        created.profilePhotoSrc=profileSrc==''?'':`${URL}/users/${profileSrc}`
    ...
    async createToken(payload:any) {
      const token=sign({payload},process.env.JWTSECRET,{expiresIn:process.env.EXPIRES})
     // console.log(token)
      return token
    }
    ...
    async findById(id:number){
        ...
        console.log(id)
       const user= await this.userRepository.findOne({where:{id:id},relations:['blockedUsers','sentMessages','writeMessages']})
       if(!user){
    ...
    async verifyOtp(verify: string,body:OtpDto) {
        console.log(verify)
           const foundOtp=await this.otpRepository.findOne({where:{token:verify}})
           ...
           if(foundOtp.expiresIn<new Date()){
            ...
            throw new HttpException('your verification code is wrong',HttpStatus.BAD_REQUEST)
           }
    ...
    async checkBlockStatus(sender:User,reciver:User){
        ...
        for (let id of blockUserIds){
            if(sender.id===id){
                throw new HttpException('you are blocked by user',HttpStatus.BAD_REQUEST)
            }
        }
    }
    ...
    async login(phone:string){
        try{
        const user= await this.userRepository.findOneOrFail({where:{phoneNumber:phone}})

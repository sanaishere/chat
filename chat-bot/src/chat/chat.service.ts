...
export class ChatService {
  ...
  private userService:UsersService){}
    ...
    async createChat(body:CreateChatDto,users:User[]){
        let chat=await this.chatRepository.findOne({where:{name:body.name},relations:{messages:true}})
        if(!chat){
    ...
    async getchatMessages(id:number){
        ...
        const chatMessages=await this.messageRepository.createQueryBuilder('message')
        ...
        .getMany()
        const messages=await Promise.all(chatMessages.map(async(message)=>{
            return{
    ...
    async checkChat(users:User[]){
        ...
        try{
         chat=await this.chatRepository.createQueryBuilder('chat')
    ...
    async deleteFromMessage(chatId:number){
        let messages=await this.messageRepository.find({where:{chat:{id:chatId}},relations:{chat:true}})
        if(messages){
      ...
      async checkAdmission(user:User,chatId:number){
        ...
        if(!userIds.includes(user.id)){
            throw new HttpException('you dont have access to see/change this chat',HttpStatus.UNAUTHORIZED)
        }
      }
     ...
     async deleteGroupChats(name:string){
        const founded=await this.chatRepository.findOne({where:{name:name}})
        if(founded){
     ...
     async deletechatsForRemovedUser(name:string,removed:User){
        let founded=await this.chatRepository.findOne({where:{name:name}
